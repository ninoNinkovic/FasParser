...
[`Version 1.2.1`] 2017-03-16
	...
	* improve the sort function with user provided ID list;
	* improve the extraction function for large FASTA file;
	* add Clear buttons for convenient operation;
	* optimize codes (flexibility and simplicity);
	* fix some bugs
	* add version check function;
	* improve align function for folder with space(s)

[`Version 1.2.0`] 2017-03-11

	* add classification function in the "Sort-ID" section;
	* add ID-rename function in "Sort-ID" section;
	* improve the alignment cut function in "Fas-Filter" section ;
	* improve the alignment filtration based on seq length;
	* fix bugs
